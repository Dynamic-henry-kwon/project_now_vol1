...
<body>
		...
		<div id="boxWapper">
			<div class="black_bg" style="diplay: block;">
				<div id="registTodoPop" class="popup"
					style="display: block; margin-top: -144px">
	...
	<script type="text/javascript">
		...
		for ( var scheduleObj in scheduleCodeObj) {
			if (scheduleObj == "수면시간" || scheduleObj == "활동시간") {
				sleepScheduleList.push([ scheduleObj,
						scheduleCodeObj[scheduleObj][1] ]);
		...
		var bilboardChartOnclick = function(d, i) {
			...
			if (d.id != '미등록') {
				if (depth == 2) {
					popUpTodoRegist(scheduleCodeObj[scheduleName][0],
							scheduleCodeObj[scheduleName][1], 1);
				} else {
					depth++;
				...
				//등록
				popUpTodoRegist(scheduleCodeAndTime[0], scheduleCodeAndTime[1],
						0);
				return null;
			}
		...
		var setDetailTime = function(selectedId, value, loadList, backBtn) {
			...
			if (backBtn == null) {
				analyzeChart.$.main.select(".bb-chart-arcs-title").text(
						selectedId);
			} else {
				analyzeChart.$.main.select(".bb-chart-arcs-title")
						.text(backBtn);
			}
		...
		var popUpTodoRegist = function(schedule, timeValue, bizNum) {
			...
			var form = toDoService.createForm('registInfo',
					"/permit/todoRegForm", "post", params);
			document.body.appendChild(form);
			$('#registTodoPop').css('margin-top',
					-($('#registTodoPop').find('.pop_area').innerHeight() / 2));
		}
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			//할일 등록으로 이동하기 위한 submit 이벤트 동적바인딩
